...
from utils.validators import validate_employee_id, validate_employee_name, validate_leave_balance, validate_leave_days, employee_exists
...
def add_employee(employee_id : int, employee_name : str) -> bool :
    ...
    try:
        ...
        if employee_exists(employee_id=employee_id, employees=employees):
            return False
        employee = Employee(employee_id=employee_id, employee_name=employee_name, leave_balance=config["policy"]["annual_leaves"])
        logger.info(
        ...
        logger.info(
            f"Employee {employee.employee_id} added to Employee List"
        )
        write_json(config["Storage"]["employee_data"],data=employees)
        logger.info(
            "Employees wrtien to json file"
        )
        return True
...
def apply_leave(employee_id:int, leave_days:int,reason:str, applied_date:date) -> bool:
    """
    ...
    try:
        ...
        if not validate_leave_days(leave_days=leave_days):
            return False
        leave = Leave(employee_id=employee_id, leave_days=leave_days,reason=reason, applied_date=applied_date)
        logger.info(
            f"Leave record created for employee_id : {leave.employee_id}"
        )
        leaves = read_json(config["Storage"]["leave_history"])
        leave_json = leave.to_dict()
        for emp in employees:
            if employee_id == emp["employee_id"]:
                if validate_leave_balance(requested_leave=leave_days, available_leave=emp["leave_balance"]) :
                    ...
                    logger.warning(f"Employee {emp['employee_id']}Leaves has Less than requested")
                    return False
...
def view_balance(employee_id: int) -> int | None:
    try:
        ...
        logger.exception(
            f"Failed to Retrieve balence for employee_id {employee_id}"
        )
        raise
...
def view_leave_history(employee_id: int) -> list[Leave]:
    try:
        ...
        logger.exception(
            f"Failed to Retrieve leaves for employee_id {employee_id}"
        )
        raise
...
def list_employees() -> list[Employee]:
    try:
        ...
        logger.info(
            f"Retrieved {len(employees)} Employee Records"
        )
        return employees
    except Exception:
        logger.exception(
            "Failed to Retrieve list of  employees"
        )
        raise